...
\class Color
\ingroup Util
Provides functions for working with color. New instances can be created with `new Color(input)` or just `Color(input)`.
Static functions are invoked with `Color.` qualifier.

Library originally from [TinyColor](https://github.com/bgrins/TinyColor).
This object can also be invoked with `tinycolor()`.

The library has been extended with extra functions described below. For the rest of the documentation, with examples,
...
\fn Color mixWith(Color color2, number amount)
\memberof Color
Returns a new color with this color and `color2` mixed by `amount`. `amount` should be in range of 0-100.

\fn Color tint(number amount)
\memberof Color
Returns a new color with this color tinted by `amount`.  `amount` should be in range of 0-100.

\fn Color shade(number amount)
\memberof Color
Returns a new color with this color shaded by `amount`.  `amount` should be in range of 0-100.

\fn Color polyad(number n)
\memberof Color
Returns an array of up to `n` colors harmonious with this color. Same as `triad()` and `tetrad()` when called with values 3 and 4, respectively.

\fn bool isDarkerThan(Color secondColor)
\memberof Color
The color with less brightness value is considered as Darker.

\fn bool isLighterThan(Color secondColor)
\memberof Color
...
\fn bool isWarmerThan(Color secondColor)
\memberof Color
...
\fn bool isCoolerThan(Color secondColor)
\memberof Color
...
tinycolor.prototype = {
    ...
    getSaturation: function () {
      ...
      return luminance === 1 ? 0 : (maxRGB - minRGB) / (1 - mathAbs(2 * luminance - 1));
    },
    ...
    toName: function() {
        if (this._a === 0 && this._r === 255 && this._g === 255 && this._b === 255) {
            return "transparent";
        }
    ...
    // mod - MP {
    mixWith: function() {
        return this._applyModification(mix, arguments);
    },
    tint: function() {
    ...
    /**
     * @fn isDarkerThan(secondColor)
     * @memberof Color
     * The color with less brightness value is considered as Darker.
     * @param {tinycolor} secondColor
     */
    isDarkerThan: function (secondColor) {
        return this.getBrightness() < secondColor.getBrightness();
    },
    /**
     * @fn isLighterThan(secondColor)
     * @memberof Color
     * Return the opposite value of isDarker().
     * @param {tinycolor} secondColor
     */
    isLighterThan: function (secondColor) {
        return !this.isDarker(secondColor);
    },
    /**
     * @fn isWarmerThan(secondColor)
     * @memberof Color
     * Returns true if the color is warmer i.e. have more red value.
     * If both colors have the same red value the one with less green and blue values is considered warmer.
     * @param {tinycolor} secondColor
     */
    isWarmerThan: function (secondColor) {
    ...
    /**
     * @fn isCoolerThan(secondColor)
     * @memberof Color
     * Return the opposite value of isWarmer().
     * @param {tinycolor} secondColor
     */
    isCoolerThan: function (secondColor) {
...
// mod - MP {
tinycolor.prototype.originalInput = tinycolor.prototype.getOriginalInput;
tinycolor.prototype.format        = tinycolor.prototype.getFormat;
tinycolor.prototype.alpha         = tinycolor.prototype.getAlpha;
tinycolor.prototype.brightness    = tinycolor.prototype.getBrightness;
tinycolor.prototype.luminance     = tinycolor.prototype.getLuminance;
tinycolor.prototype.saturation    = tinycolor.prototype.getSaturation;
tinycolor.prototype.hsv           = tinycolor.prototype.toHsvString;
tinycolor.prototype.hsl           = tinycolor.prototype.toHslString;
...
// mod - MP {
/*!
    \fn Color mix(Color color1, Color color2, number amount)
    \memberof Color
    \static
...
/*!
    \fn Color tint(Color color, number amount)
    \memberof Color
    \static
    Returns a new Color of `color` by `amount`. `amount` should be in range of 0-100.
...
/*!
    \fn Color shade(Color color, number amount)
    \memberof Color
    \static
    Returns a new Color with `color` shaded by `amount`. `amount` should be in range of 0-100.
...
/*!
    \fn Color complement(Color color)
    \memberof Color
    \static
...
/*!
    \fn Color polyad(Color color, number n)
    \memberof Color
    \static
...
function analogous(color, results, slices) {
    ...
    slices = slices && slices > 0 ? slices : 30;     // mod - MP
    var hsl = tinycolor(color).toHsl();
    var part = 360 / slices;
...
/**
    \fn bool isDarker(Color color1, Color color2)
    \memberof Color
    \static
    The color with less brightness value is considered as Darker.
    This is a static function. Use it like `Color.isDarker("white", "black")`
*/
tinycolor.isDarker = function (color1, color2) {
    return color1.isDarkerThan(color2);
};

/**
    \fn bool isLighter(Color color1, Color color2)
    \memberof Color
    \static
    Return the opposite value of isDarker().
    This is a static function. Use it like `Color.isLighter("white", "black")`
 */
tinycolor.isLighter = function (color1, color2) {
    return color1.isLighterThan(color2);
};

/**
    \fn bool isWarmer(Color color1, Color color2)
    \memberof Color
    \static
 ...
 */
tinycolor.isWarmer = function (color1, color2) {
    return color1.isWarmerThan(color2);
};

/**
    \fn bool isCooler(Color color1, Color color2)
    \memberof Color
    \static
    Return the opposite value of isWarmer().
    This is a static function. Use it like `Color.isCooler("red", "blue")`
 */
tinycolor.isCooler = function (color1, color2) {
    return color1.isCoolerThan(color2);
};
// } mod - MP
...
// Take input from [0, n] and return it as [0, 1], except if max == 360 then wrap to [0, 360]
function bound01(n, max) {
    if (isOnePointZero(n)) { n = "100%"; }

    var processPercent = isPercentage(n);
    // If range [0,360] then do nothing.
    n = max === 360 ? n : mathMin(max, mathMax(0, parseFloat(n)));  // mod - MP
    ...
    if (max === 360) {
        ...
        return (n < 0 ? n % max + max : n % max) / 360.0;
    }
